<?php
  class Modelo_Administrador{
    ...
    function Camb_ultimo_fech_pagp($idalum,$fechmay,$fechmas){
      $sql = "UPDATE alumno SET fechaRegisto ='$fechmay' ,fechaUpdate = '$fechmas',stadoPago ='PAGADO' where idalumno ='$idalum'";
      if ($consulta = $this->conexion->conexion->query($sql)) {
    ...
    function Pagos_mensuales_Alumnos($alumnombre,$pagovect,$vecFech){
      $sql="insert into registropago (alumnonombre, montopago,description, fechasPagados, fechaUpdate) values ('$alumnombre','$pagovect','PENCION','$vecFech', NOW())";
      if ($consulta = $this->conexion->conexion->query($sql)) {
