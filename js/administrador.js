...
$('#tabla_matricula').on('click', '.imprimir', function() {
    ...
    if (table_matricula.row(this).child.isShown()) {
     ...
     /*window.open("../vista/reporte/vista_imprimir.php?id="+parseInt(data.idalumno)+
        "#zoom=100%","report","scrollbars=NO");
    ...
    "url": "../controlador/administrador/controlador_eliminar_aulumno.php",
        ...
        if (resp > 0) {
            Swal.fire({
                ...
                text: "Una vez hecho esto se eliminaran los datos",
                icon: 'warning',
            ...
            Swal.fire("Mensaje De Error", "no se pudo completar,ya esta ocupado por un grado", "error");
        }
    })
...
function matriculaAlunos(){
    ...
    var tiempo_inicio = performance.now();
        ...
        $.ajax({
             ...
             data: {
                 ...
                 direccion:direccion
                    ...
                    var tiempo_fin = performance.now();
               if (resp > 0) {
                   if (resp == 1) {
        ...
        var tiempo_ejecucion = tiempo_fin - tiempo_inicio;

        // Convierte el tiempo de ejecución a segundos
        var tiempo_ejecucion_segundos = tiempo_ejecucion / 1000;

        // Muestra el tiempo de ejecución en segundos en un alert
        alert("Tiempo de ejecución: " + tiempo_ejecucion_segundos + " segundos");
    });
